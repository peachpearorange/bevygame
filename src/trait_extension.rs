...
pub(crate) trait MeshExt {
  ...
  fn search_in_children<'a>(parent: Entity,
...
impl MeshExt for Mesh {
  ...
  fn transformed(&self, transform: Transform) -> Mesh {
    // let new_coords = map(|coords| transform.transform_point(coords.into()).into(),
    //                      self.read_coords(Mesh::ATTRIBUTE_POSITION.clone()));
    // let new_normals = map(|normal| transform.rotation.mul_vec3(normal.into()).into(),
    //                       self.read_coords(Mesh::ATTRIBUTE_NORMAL.clone()));
    // self.attribute_mut
    self.clone().tap_mut(|s| s.transform(transform))
  }

  fn read_coords_mut(&mut self, id: impl Into<MeshVertexAttributeId>) -> &mut Vec<[f32; 3]> {
    // Guaranteed by Bevy for the current usage
    match self.attribute_mut(id)
  ...
  fn search_in_children<'a>(parent: Entity,
    ...
    if let Ok(children) = children_query.get(parent) {
      let result: Vec<_> =
        children.iter()
                .filter_map(|entity| {
                  mesh_handles.get(*entity).ok().map(|mesh| (*entity, mesh))
                })
      ...
      let inner_result =
        children.iter()
                ...
                .collect();
      // let vec_append = |v1, v2| v1.tap_mut(|v| v.append(v2));
      result.tap_mut(|r| r.append(inner_result))
...
impl F32Ext for f32 {
  #[inline]
  fn is_approx_zero(self) -> bool { self.abs() < 1e-5 }

  #[inline]
  fn squared(self) -> f32 { self * self }

  #[inline]
  fn lerp(self, other: f32, ratio: f32) -> f32 { self.mul_add(1. - ratio, other * ratio) }
}
