pub(crate) use crate::foxtrot::world_interaction::dialog::resources::{CurrentDialog, Dialog,
                                                                      DialogEvent, DialogId,
                                                                      NextPage};
use {crate::foxtrot::{file_system_interaction::{asset_loading::DialogAssets,
                                                config::GameConfig},
                      player_control::actions::{ActionsFrozen, PlayerAction},
                      world_interaction::{condition::{ActiveConditions,
                                                      ConditionAddEvent, ConditionId},
                                          dialog::resources::Page},
                      GameState},
...
fn set_current_dialog(mut commands: Commands,
  ...
  for dialog_event in dialog_events.iter() {
    let path = Path::new("dialogs").join(&dialog_event.dialog.0.clone())
    ...
    let dialog_handle = match dialog_handles.dialogs.get(&path) {
      Some(handle) => handle,
      None => {
        error!("Failed to load dialog \"{}\": No such dialog. Available dialog: {:?}",
               path,
               dialog_handles.dialogs.keys());
        continue;
      }
    };
    let dialog = dialogs.get(dialog_handle)
    ...
    commands.insert_resource(CurrentDialog { source: dialog_event.source,
...
#[sysfail(log(level = "error"))]
fn show_dialog(mut commands: Commands,
...
fn present_choices(ui: &mut egui::Ui,
  ...
  match next_page {
    ...
    NextPage::Choice(choices) => {
      ...
      for (index, (choice_id, choice)) in choices.iter()
                                                 .filter(|(choice_id, choice)| {
                                                   choice.is_available(active_conditions)
                                                   && !was_just_picked(current_dialog,
                                                                       choice_id)
                                                 })
                                                 .enumerate()
      {
...
fn get_dialog_window() -> egui::Window<'static> {
  egui::Window::new("Dialog").anchor(egui::Align2::CENTER_BOTTOM, egui::Vec2::new(0., -30.))
                             .collapsible(false)
                             .resizable(false)
                             .title_bar(false)
                             .frame(egui::Frame { fill:
                                                    egui::Color32::from_black_alpha(230),
                                                  inner_margin:
                                                    egui::style::Margin::same(25.),
                                                  rounding: egui::Rounding::same(30.0),
                                                  ..default() })
}

fn set_dialog_style(style: &mut egui::Style) {
  style.text_styles = [(Body, FontId::new(16.0, Proportional)),
                       (Button, FontId::new(14.0, Proportional))].into();
...
fn create_dialog_rich_text(page: &Page, elapsed_time: f32, config: &GameConfig) -> String {
  ...
  let letters_to_display =
    (base_letters_per_second * page.talking_speed * elapsed_time) as usize;
