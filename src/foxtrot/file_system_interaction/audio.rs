use {crate::foxtrot::{file_system_interaction::asset_loading::AudioAssets, GameState},
     bevy::prelude::*,
     bevy_kira_audio::prelude::{Audio, *}};
...
fn init_audio(mut commands: Commands, audio_assets: Res<AudioAssets>, audio: Res<Audio>) {
  audio.pause();
  let handle = audio.play(audio_assets.walking.clone())
                    .looped()
                    .with_volume(0.8)
                    .handle();
