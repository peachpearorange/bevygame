use {crate::foxtrot::{file_system_interaction::asset_loading::LevelAssets,
                      level_instantiation::spawning::GameObject,
                      world_interaction::{condition::ActiveConditions,
                                          dialog::CurrentDialog,
                                          interactions_ui::InteractionOpportunities}},
     anyhow::{Context, Result},
     bevy::{prelude::*, reflect::TypeUuid},
     bevy_mod_sysfail::macros::*,
     serde::{Deserialize, Serialize},
     spew::prelude::*,
     std::{fs, iter, path::Path}};
...
fn save_world(mut save_requests: EventReader<WorldSaveRequest>,
              spawn_query: Query<(&GameObject, Option<&Transform>)>)
              -> Result<()> {
  for save in save_requests.iter() {
    let scene = save.filename.clone();
    let valid_candidates: Vec<_> =
      iter::once(scene.clone()).chain((1..).map(|n| format!("{0}-{n}", scene.clone())))
                               .map(|filename| {
                                 Path::new("assets").join("levels")
                                                    .join(filename)
                                                    .with_extension("lvl.ron")
                               })
                               .map(|path| (path.clone(), fs::try_exists(path).ok()))
                               .take(10)
                               .filter_map(|(path, maybe_exists)| {
                                 maybe_exists.map(|exists| (path, exists))
                               })
                               .collect();
                      ...
                      .next()
    {
      ...
      fs::write(path, serialized_world).unwrap_or_else(|e| {
                                         error!("Failed to save level \"{}\": {}", scene, e)
                                       });
      info!("Successfully saved level \"{}\" at {}",
            scene,
            path.to_string_lossy());
    } else {
      error!("Failed to save level \"{}\": Already got too many saves with this name",
             scene);
    }
  }
  Ok(())
...
fn load_world(mut commands: Commands,
  ...
  for load in load_requests.iter() {
    let path =
      Path::new("levels").join(load.filename.clone())
                         .with_extension("lvl.ron")
                         .to_str()
                         .with_context(|| {
                           format!("Failed to convert path to string for filename: {}",
                                   load.filename)
                         })?
                         .to_string();
    let handle = match level_handles.levels.get(&path) {
      Some(handle) => handle,
      None => {
        error!("Failed to load scene \"{}\": No such level. Available levels: {:?}",
               path,
               level_handles.levels.keys());
        continue;
      }
    };
    let spawn_events = &levels.get(handle)
    ...
    commands.insert_resource(CurrentLevel { scene: load.filename.clone() });
  ...
  let objects: Vec<_> =
    spawn_query.iter()
               ...
               .map(|(game_object, transform)| {
                 SpawnEvent::with_data(*game_object,
