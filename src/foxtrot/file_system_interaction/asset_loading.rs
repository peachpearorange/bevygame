use {crate::foxtrot::{file_system_interaction::{config::GameConfig,
                                                level_serialization::SerializedLevel},
                      world_interaction::dialog::Dialog,
                      GameState},
     anyhow::{Context, Result},
     bevy::{prelude::*, utils::HashMap},
     bevy_asset_loader::prelude::*,
     bevy_common_assets::{ron::RonAssetPlugin, toml::TomlAssetPlugin},
     bevy_egui::{egui, egui::ProgressBar, EguiContexts},
     bevy_kira_audio::AudioSource,
     bevy_mod_sysfail::macros::*,
     iyes_progress::{ProgressCounter, ProgressPlugin}};
...
fn show_progress(progress: Option<Res<ProgressCounter>>,
  ...
  if let Some(progress) = progress.map(|counter| counter.progress()) {
    ...
    egui::CentralPanel::default().show(egui_contexts.ctx_mut(), |ui| {
        ...
        ui.add(ProgressBar::new(progress.done as f32 / progress.total as f32).animate(true));
        ui.add_space(100.0);
...
fn update_config(mut commands: Commands,
