use {crate::foxtrot::{file_system_interaction::level_serialization::{CurrentLevel,
                                                                     WorldLoadRequest},
                      ...
                      world_interaction::{condition::ActiveConditions,
                                          dialog::{CurrentDialog, DialogEvent}},
                      GameState},
  ...
  app.add_event::<GameSaveRequest>()
        ...
        .add_systems(
            (
                ...
                .chain()
                .in_set(Update(GameState::Playing)),
        );
}
...
fn handle_load_requests(mut commands: Commands,
  ...
  for load in load_events.iter() {
    ...
    let save_model: SaveModel = match ron::from_str(&serialized) {
      ...
      Err(e) => {
        error!("Failed to deserialize save {:?} at {:?}: {}",
               &load.filename, path, e);
        continue;
      }
    };
    loader.send(WorldLoadRequest { filename: save_model.scene });
...
#[sysfail(log(level = "error"))]
fn handle_save_requests(mut save_events: EventReader<GameSaveRequest>,
  ...
  for save in save_events.iter() {
    for player in &player_query {
      let dialog_event = dialog.clone()
                               .map(|dialog| DialogEvent { dialog: dialog.id,
                                                           source: dialog.source,
                                                           page: Some(dialog.current_page) });
      let save_model = SaveModel { scene: current_level.scene.clone(),
      ...
      fs::write(&path, serialized).unwrap_or_else(|e| {
                                    error!("Failed to write save {filename}: {e}")
                                  });
