use {crate::foxtrot::{file_system_interaction::game_state_serialization::{GameLoadRequest,
                                                                          GameSaveRequest},
                      foxtrot::file_system_interaction::level_serialization::{WorldLoadRequest,
                                                                              WorldSaveRequest},
                      level_instantiation::spawning::GameObject,
                      player_control::camera::ForceCursorGrabMode,
                      GameState},
     anyhow::{Context, Result},
     bevy::{prelude::*, window::CursorGrabMode},
     bevy_editor_pls::{editor::{Editor, EditorEvent},
                       editor_window::EditorWindow,
                       AddEditorWindow},
     bevy_egui::{egui, egui::ScrollArea},
     bevy_mod_sysfail::macros::*,
     bevy_prototype_debug_lines::DebugLines,
     bevy_rapier3d::prelude::*,
     oxidized_navigation::NavMesh,
...
impl EditorWindow for DevEditorWindow {
  ...
  fn ui(world: &mut World,
        ...
        ui: &mut egui::Ui) {
    let state = cx.state_mut::<DevEditorWindow>()
    ...
    ui.add_enabled_ui(!state.level_name.is_empty(), |ui| {
        ui.horizontal(|ui| {
            if ui.button("Save").clicked() {
              world.send_event(WorldSaveRequest { filename: state.level_name.clone() })
            }
            if ui.button("Load").clicked() {
              world.send_event(WorldLoadRequest { filename: state.level_name.clone() });
    ...
    ui.horizontal(|ui| {
        ...
        if ui.button("Save").clicked() {
          world.send_event(GameSaveRequest { filename: filename.clone() })
        }
        if ui.button("Load").clicked() {
    ...
    if ui.button("Spawn").clicked() {
      world.send_event(SpawnEvent::with_data(state.spawn_item, Transform::default()));
    }
                          ...
                          .show(ui, |ui| {
                            ui.vertical(|ui| {
                                for item in GameObject::iter() {
                                  ui.radio_value(&mut state.spawn_item,
                                                 item,
                                                 format!("{item:?}"));
                                }
                              });
...
impl Default for DevEditorState {
  fn default() -> Self {
    Self { level_name: "old_town".to_owned(),
           save_name: default(),
           spawn_item: default(),
...
fn handle_debug_render(state: Res<Editor>,
                       mut debug_render_context: ResMut<DebugRenderContext>)
                       -> Result<()> {
  debug_render_context.enabled = state.window_state::<DevEditorWindow>()
                                      .context("Failed to read dev window state")?
                                      .collider_render_enabled;
  Ok(())
}

fn set_cursor_grab_mode(mut events: EventReader<EditorEvent>,
...
#[sysfail(log(level = "error"))]
fn handle_navmesh_render(state: Res<Editor>,
                         nav_mesh: Res<NavMesh>,
                         mut lines: ResMut<DebugLines>)
                         -> Result<()> {
  if !state.window_state::<DevEditorWindow>()
           .context("Failed to read dev window state")?
  ...
  if let Ok(nav_mesh) = nav_mesh.get().read() {
    for (tile_coord, tile) in nav_mesh.get_tiles().iter() {
      let tile_color = Color::Rgba { red: 0.0,
