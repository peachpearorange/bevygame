...
use {crate::foxtrot::{file_system_interaction::asset_loading::TextureAssets, GameState},
     ...
     bevy::{asset::HandleId,
            ...
            render::{mesh::MeshVertexBufferLayout,
                     render_resource::{AsBindGroup, Face::Front,
                                       RenderPipelineDescriptor, ShaderRef, ShaderType,
...
fn setup_shader(mut commands: Commands,
                mut glow_materials: ResMut<Assets<GlowyMaterial>>,
                mut skydome_materials: ResMut<Assets<SkydomeMaterial>>,
                texture_assets: Res<TextureAssets>) {
  let glowy =
    glow_materials.add(GlowyMaterial { env_texture: texture_assets.glowy_interior.clone() });
  let skydome =
    skydome_materials.add(SkydomeMaterial { env_texture: texture_assets.sky.clone() });

  commands.insert_resource(Materials { repeated: HashMap::new(),
                                       glowy,
                                       skydome });
}
...
impl Material for GlowyMaterial {
  fn fragment_shader() -> ShaderRef { "shaders/glowy.wgsl".into() }
}
...
impl Material for SkydomeMaterial {
  fn fragment_shader() -> ShaderRef { "shaders/skydome.wgsl".into() }
...
impl Material for RepeatedMaterial {
  fn fragment_shader() -> ShaderRef { "shaders/repeated.wgsl".into() }
}
...
pub(crate) fn set_texture_to_repeat(mut commands: Commands,
  ...
  for (name, children) in &added_name {
    if let Some(captures) = REPEAT_REGEX.captures(&name.to_lowercase()) {
      let repeats =
        Repeats { horizontal: captures[1].parse().context("Failed to parse repeat")?,
      ...
      for child in children.iter() {
        if let Ok(standard_material_handle) = material_handles.get(*child) {
          let standard_material =
            standard_materials.get(standard_material_handle)
          ...
          let repeated_material =
            materials.repeated.entry(key).or_insert_with(|| {
