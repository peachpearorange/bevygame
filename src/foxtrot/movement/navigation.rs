#[cfg(feature = "dev")]
use crate::foxtrot::dev::dev_editor::DevEditorWindow;
#[cfg(feature = "dev")] use anyhow::Context;
#[cfg(feature = "dev")]
use bevy_prototype_debug_lines::DebugLines;
use {crate::foxtrot::{level_instantiation::spawning::objects::npc,
                      movement::general_movement::{GeneralMovementSystemSet, Walking},
     ...
     oxidized_navigation::{query::{find_path, perform_string_pulling_on_path},
...
pub(crate) fn navigation_plugin(app: &mut App) {
  app.add_plugin(OxidizedNavigationPlugin)
     ...
     .insert_resource(NavMeshSettings { cell_width: CELL_WIDTH,
                                        ...
                                        max_edge_length: 100 })
     .add_system(query_mesh.before(GeneralMovementSystemSet)
...
fn query_mesh(mut with_follower: Query<(&Transform, &mut Walking),
    ...
    for (follower_transform, mut walking) in &mut with_follower {
      for player_transform in &with_player {
        ...
        if let Ok(path) = find_path(&nav_mesh, &nav_mesh_settings, from, to, None, None) {
          ...
          #[cfg(feature = "dev")]
          if editor_state.window_state::<DevEditorWindow>()
                         .context("Failed to get dev window state")?
                         .navmesh_render_enabled
          {
            draw_path(&path, &mut lines, Color::RED);
          }
          let dir = path.into_iter()
                        .map(|next_point| {
                          (next_point - from).split(follower_transform.up())
                                             .horizontal
                        })
